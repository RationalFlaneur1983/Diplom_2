...
class TestCreateOrder:
    ...
    def test_invalid_create_order_without_ingredients_(self, registered_user):
        ...
        response = requests.post(url, headers=headers, json=payload)
        response_data = response.json()
        assert response.status_code == 400 and response_data['message'] == 'Ingredient ids must be provided'
...
class TestGetOrders:
    ...
    def test_invalid_get_unauthorized_user_order(self):
        ...
        response_data = response.json()
        assert response.status_code == 401 and response_data['message'] == 'You should be authorised'
